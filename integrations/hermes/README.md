...
This integration installs one Hermes plugin named `genixbit-agents-router` instead
...
the `genixbit-agents-router` plugin/router and load only the specialists needed for
...
```text
${HERMES_HOME:-~/.hermes}/plugins/genixbit-agents-router
```

It then enables `genixbit-agents-router` under `plugins.enabled` in the Hermes
