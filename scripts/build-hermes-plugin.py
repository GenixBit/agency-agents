...
PLUGIN_NAME = "genixbit-agents-router"
...
def readme(agent_count: int) -> str:
    return textwrap.dedent(
        ...
        ```text
        Use the genixbit-agents-router plugin. Search the Genixbit roster for the right
        ...
        full Genixbit roster and do not add genixbit-agents to skills.external_dirs.
        ```

        Example:

        ```text
        For this Data Swami build, use the genixbit-agents-router plugin to pick
        relevant Genixbit specialists. Search first, then delegate to selected agents
