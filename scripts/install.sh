...
#   openclaw     -- Copy workspaces to ~/.openclaw/genixbit-agents/
...
tool_label() {
  case "$1" in
    ...
    openclaw)    printf "%-14s  %s" "OpenClaw"     "(~/.openclaw/genixbit-agents)" ;;
...
install_openclaw() {
  ...
  local dest; dest="$(resolve_dest openclaw "${HOME}/.openclaw/genixbit-agents")"
  local count=0
...
ensure_hermes_plugin_enabled() {
  ...
  plugin="genixbit-agents-router"
  mkdir -p "$hermes_home"
  backup="${config}.bak.genixbit-agents-plugin.$$"
...
install_hermes() {
  local src="$INTEGRATIONS/hermes/genixbit-agents-router"
  local hermes_home; hermes_home="$(hermes_home_dir)"
  local dest; dest="$(resolve_dest hermes "${hermes_home}/plugins/genixbit-agents-router")"
  ...
  # the genixbit-agents-router subdir so we never rm -rf a shared plugins dir that
  # holds other plugins.
  if [[ "$(basename "$dest")" != "genixbit-agents-router" ]]; then
    dest="${dest%/}/genixbit-agents-router"
  fi
  [[ -f "$src/plugin.yaml" && -f "$src/__init__.py" && -f "$src/data/agents.json" ]] || {
    err "integrations/hermes/genixbit-agents-router missing. Run ./scripts/convert.sh --tool hermes first."
    return 1
  }
  mkdir -p "$(dirname "$dest")"
  # Safety net: only ever remove our own plugin directory, never a parent.
  if [[ "$(basename "$dest")" != "genixbit-agents-router" ]]; then
    err "Hermes: refusing to remove '$dest' — expected an genixbit-agents-router directory."
    return 1
  fi
  rm -rf "$dest"
  if $USE_LINK; then
